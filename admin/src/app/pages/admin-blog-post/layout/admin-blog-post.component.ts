...
export class AdminBlogPostComponent implements OnInit {
	...
	ngOnInit() {
		this.blogAction =
			this.activatedRoute.snapshot.params['id'] === 'create'
				? BLOG_ACTION.CREATE
				: BLOG_ACTION.EDIT;
		this.getFormGroup();
	...
	public onSubmit(data: {
			...
			for (let i = 0; i < description.length - 6; ++i) {
				if (
					...
					if (imageAltsArr[imageCounter]) {
						body.description.splice(
							i,
							6,
							`<img alt="${imageAltsArr[imageCounter]}"`
						);
						imageCounter++;
						i += 3;
					}
		...
		if (postAction === BLOG_ACTION.CREATE) {
			...
			forkJoin([
				...
				this.blogService
					.uploadImage(this.getFormData({ image }), _id)
					.pipe(first()),
			])
				.pipe(
					this.hotToastService.observe({
