...
export class AdminCompaniesListComponent implements OnInit {
	...
	private paginationParams: PaginationParamsModel = {
		offset: 0,
		limit: this.PRODUCTS_LIMIT,
	};
	private companyListSource: BehaviorSubject<any[]> = new BehaviorSubject<
		any[]
	>([]);
	private companyListLengthSource: BehaviorSubject<number> =
	...
	ngOnInit(): void {
		this.form
			...
			.valueChanges.pipe(
				switchMap((value) =>
					this.marketService.getCompanyByName(value, this.paginationParams)
				)
			)
	...
	toggleCompaniesPage(pageNumber: number) {
		...
		this.marketService
			.getCompanyByName(this.form.get('q').value, this.paginationParams)
			.subscribe((data) => {
