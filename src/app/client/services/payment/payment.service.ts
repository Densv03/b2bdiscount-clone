...
export class PaymentService {
	...
	public updateCustomerProfile(data: any): Observable<any> {
		...
		return this.apiService.post(
			'authorize-net/update-payment-by-profile',
			body
		);
	}

	public createPaymentByProfile(body: {
		amount: number | string;
		type: string;
		quotesNumb?: number;
	}): Observable<any> {
		return this.apiService.post(
			'authorize-net/create-payment-by-profile',
			body
		);
	}
	...
	public getTokenForPurchaseConfirm(body: any): Observable<any> {
		return this.apiService.post(
			'authorize-net/get-hosted-payment-page-request',
			body
		);
	}
}
