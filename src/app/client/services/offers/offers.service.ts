...
export class OffersService {
	...
	private savedOffersSource: BehaviorSubject<any[]> = new BehaviorSubject<
		any[]
	>([]);

	private offersLength: BehaviorSubject<number> = new BehaviorSubject<number>(
		0
	);
	private savedOffersLength: BehaviorSubject<number> =
	...
	public deleteRfq(id: string) {
		return this._apiService.delete(`tradeBid/delete-rfq-admin`, {
			body: { rfqId: id },
		});
	}
	...
	public getMyOffers(): Observable<any> {
		this._apiService.get('my/offers?limit=1').subscribe((myOffers: any) => {
			this._offersStore.update({
			...
			tap(() => {
				...
				this._offersStore.update({
					savedOffers: savedOffers.filter(
						(savedOffer: any) => savedOffer._id !== id
					),
