...
export class ClientBlogArticleComponent implements AfterViewInit, OnDestroy {
	...
	ngAfterViewInit() {
		this.getArticle$().pipe(
			...
			catchError(() => {
		...
		).subscribe(({blog: {_id, createdAt, updatedAt, title, image, author, description}}) => {
				...
				this.seoService
			.addBlogMicroMarkup(this._document, window.location.href, title, new Date(createdAt), new Date(updatedAt), imgArray, author, description);
			});
	}
	...
	public getArticle$(): Observable<any> {
		return this._activatedRoute.paramMap.pipe(
			...
			map((article) => {
				return {blog: article.blog, anotherBlogs: article.anotherBlogs.slice(0, -1)}
			})
		);
	}
	...
	ngOnDestroy() {
		this._document.querySelector("[data-chats-widget-id]") && this._document.querySelector("[data-chats-widget-id]").remove();
		this._document.querySelector("[id='micro-markup']") && this._document.querySelector("[id='micro-markup']").remove();
	}
}
