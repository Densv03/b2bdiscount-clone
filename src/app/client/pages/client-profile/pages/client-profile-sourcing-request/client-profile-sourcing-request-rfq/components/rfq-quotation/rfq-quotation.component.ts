...
export class RfqQuotationComponent implements OnInit {
	...
	constructor(
		...
		this.unit$ = this.unitsService
			.getUnits()
			.pipe(map((data) => [...data, { displayName: 'Other' }]));
	...
	public ngOnInit(): void {
		...
		this.sourcingRequestService
			...
			.pipe(
				switchMap((rfq) =>
					this.categoriesService.getCategoryNameById(rfq.category)
				),
				untilDestroyed(this)
			)
	...
	public patchValueToForm(): void {
		this.sourcingRequestService.getRfqByIdAdmin(this.rfqId).subscribe((el) => {
			this.form.patchValue({
				quotationForm: {
	...
	public submitForm(form: FormGroup): void {
		...
		this.clientProfileSourcingRequestService.editItem(body).subscribe({
			error: () => this.form.reset(),
			complete: () =>
				this.router.navigate(['/profile/sourcing-request/my-rfq']),
		});
	}
