...
export class ClientProfileMyOffersComponent {
    ...
    private readonly categoriesService: CategoriesService
		...
		const isLangAlreadyLoaded = this.translateService.instant("OFFERS.MARK_AS_SOLD") !== "OFFERS.MARK_AS_SOLD";
		...
		this.translateService.onLangChange.pipe(filter((e) => e.translations === "translationLoadSuccess")).subscribe((e) => {
          ...
          this.categoriesService.getCategoryNameById(offer.category)
            ...
            .subscribe(name => {
              this.mixpanelService.track('Unclaimed cargo marked as Sold', {
                'Product Sector': name,
                'Destination': [getName(offer.destination.from), getName(offer.destination.to)]
              });
            })
