...
export class CompaniesCounterComponent implements AfterViewInit {
	...
	public totalUsersCount$: Observable<number> =
		this.userService.getTotalUsersAmount();
				...
				setTimeout(() => {
					...
					if (isNaN(+res)) {
						stat.insertAdjacentHTML(
							'beforeend',
							`<div class="parent-span">${res}</div>`
						);
					} else {
						for (let i = 0; i < res.length; i++) {
							const oneMoreThanCurrent =
								parseInt(res[i]) === 9 ? 0 : +res[i] + 1;
						...
						<div class="parent-span" data-value="${res[i]}">
							...
							${Array(parseInt(res[i]) + 1)
								...
								.join('')}
							<div class="child-span gray">${
								parseInt(res[i]) + 1 == 10 ? 0 : parseInt(res[i]) + 1
							}</div>
					  </div>`
							);
						}
