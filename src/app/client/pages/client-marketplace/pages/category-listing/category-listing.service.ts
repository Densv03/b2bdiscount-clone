...
export class CategoryListingService {
	private listingCategoriesSource: BehaviorSubject<Category[]> =
		new BehaviorSubject<Category[]>([]);
	private listingCategoriesCount: BehaviorSubject<number> =
	...
	public getPresentCategories(
		queryObj?: PaginationParamsModel
	): Observable<CategoriesPaginationModel> {
		if (queryObj) {
	...
	public updatePresentCategoriesSource(
		queryParams?: PaginationParamsModel
	): void {
		this.getPresentCategories(queryParams).subscribe(
			({ categories, totalCount }: CategoriesPaginationModel) => {
