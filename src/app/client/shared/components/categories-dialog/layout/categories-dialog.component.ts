...
export class CategoriesDialogComponent implements ControlValueAccessor {
	...
	private categoriesPlaceholderSource: BehaviorSubject<string> =
		...
		this.dialog
			...
			.subscribe((categoriesList) => {
				if (categoriesList.categories.length) {
					this.categoriesPlaceholderSource.next(
						`${categoriesList.placeholder.join(', ')}...`
					);
					this.lastValue = categoriesList.categories.map(
						(category: { value: any }) => category.value
					);
					this.onChange(
