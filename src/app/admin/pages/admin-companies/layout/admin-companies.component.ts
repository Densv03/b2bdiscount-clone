...
export class AdminCompaniesComponent implements OnInit {
	...
	public updateCompaniesListOnFiltersChange(): void {
		this.filtersForm.valueChanges
			.pipe(
				map((data) => {
					if (data.registrationDate.startDate) {
						...
						data.registrationDate.startDate = new Date(
							Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())
						).toISOString();
					}
					if (data.registrationDate.endDate) {
						data.registrationDate.endDate = new Date(
							data.registrationDate.endDate
						).toISOString();
					}
					if (!data.q) {
						delete data.q;
			...
			.subscribe((filters: { [key: string]: any }) => {
				...
				this.adminCompaniesService.updateAdminCompaniesList(
					this.filteredQueryObj,
					{ ...this.removeFalsyKeysFromObject(filters) }
				);
	...
	public togglePage(page: number): void {
		...
		this.adminCompaniesService.updateAdminCompaniesList(this.filteredQueryObj, {
			...this.currentFilters,
		});
	}
